...
function initAct() {
    $(".imaCell_img img").mousedown(function () {
        var valueZ = giveZindex("img") + 1; if (giveZindex("div") > valueZ) { valueZ = giveZindex("div") + 1 }
    ...
    $(".imaCell_img").droppable({
        accept: ".imaCell_img img", drop: function () {
            changeSrc(imaSel, parseInt($(this).attr("id").substring(3, 5)));
    ...
    $(".imaCell_img img").dblclick(function (event) {
    ...
    $(".imaCell_img img").keydown(function (e) {
        if (e.which != 9) {
            if (e.which != 27) {
                if (imaSel == -1) { var valueZ = giveZindex("img") + 1; if (giveZindex("div") > valueZ) { valueZ = giveZindex("div") + 1 }; var cssObj = { "z-index": valueZ }; $(this).css(cssObj); imaSel = $(this).parent().attr("id").substring(3, 5); $(".imaCell_img img").first().focus(); }
                else {
                    changeSrc(imaSel, parseInt($(this).parent().attr("id").substring(3, 5))); var cssObj = { "border-color": "black", "border-style": "dotted", "border-width": "1px", }; $(this).parent().css(cssObj);
                    isCorrect();
...
function changeSrc(fromSrc, toSrc) {
    var celTo; var celFrom; var srcTo; var srcFrom; var nameDivTo = "ima" + toSrc + "_img"; var nameDivFrom = "ima" + fromSrc + "_img";
...
function randomSort() {
    var mat = [["0", "0", "0"], ["0", "0", "0"]];
    var mat2 = [["0", "0", "0"], ["0", "0", "0"]];
    var count = 0; var nameDiv; var cel; var rand = 0;
    for (i = 0; i < 2; i++) { for (j = 0; j < 3; j++) { if (board[i][j] != "0") { mat[i][j] = "1"; mat2[i][j] = "1"; count += 1; } } } var countTxt = count; var count1 = 0;
    for (i = 0; i < 2; i++) {
        for (j = 0; j < 3; j++) {
            if (board[i][j] != "0") {
                rand = Math.floor(Math.random() * count); count1 = 0;
                var isA = false; for (i1 = 0; i1 < 2; i1++) {
                    for (j1 = 0; j1 < 3; j1++) {
                        if (!isA) {
                            if (mat[i1][j1] != "0") {
                                if (parseInt(rand) == parseInt(count1)) {
...
function paintButtonTime() {
    if (tiTime || tiAttempts || tiScore || tiSuccesses) { clearInterval(timeInterval); }
    ...
    interval = setInterval(function () {
        contexto.strokeStyle = colorText; roundedRect(contexto, xAnim, y - 20, wAnim, 30, 5, colorBack); xAnim -= 1; wAnim += 2;
        if (wAnim > metricsW + 30) {
            clearInterval(interval); contexto.shadowColor = "black"; contexto.shadowBlur = 20; contexto.shadowOffsetX = 10; contexto.shadowOffsetY = 10; contexto.stroke(); contexto.beginPath();
            contexto.textAlign = "left"; contexto.fillStyle = colorText; contexto.fillText(textButtonTime, x, y); contexto.lineWidth = 5; contexto.stroke();
        }
    }, 1);
...
function isCorrect() {
    ...
    //    score = score + scoreInc; $("#ardoraActCanvas").attr("aria-label", messageOk); showMessage("Ok");
