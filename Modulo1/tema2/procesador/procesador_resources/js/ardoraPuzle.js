...
$(".imaCell_img img").keydown(function(e) {if (e.which!=9){if (e.which !=27){if (imaSel==-1){var valueZ=giveZindex("img")+1;if (giveZindex("div") > valueZ) {valueZ=giveZindex("div")+1};var cssObj={"z-index":valueZ};$(this).css(cssObj);imaSel=$(this).parent().attr("id").substring(3, 5);$(".imaCell_img img").first().focus();}
...
function randomSort(){var mat=[["0","0","0"],["0","0","0"]];
...
for (i=0; i <2; i++){for (j=0; j <3; j++){if (board[i][j]!="0"){rand=Math.floor(Math.random()*count);count1=0;
var isA=false;for (i1=0; i1<2; i1++){ for (j1=0; j1<3; j1++){if (!isA){ if (mat[i1][j1]!="0"){if (parseInt(rand)==parseInt(count1)){
...
for (i = 0; i <2;i++) {for (j = 0; j<3;j++) {var nameDiv="ima"+(j+1).toString()+(i+1).toString()+"_img";
