...
function paintQuest() {
  ...
  if (audioW[indexG].localeCompare("") == 0) { $("#ardoraQuest").html("<p>" + questionW[indexG] + "</p>"); } else {
    $("#ardoraQuest").html("<img id='playSound' class='imaLeft' src='Hardware_resources/media/sound.png' alt='Sound' align='left'>" + "<p>" + questionW[indexG] + "</p>");
...
function randomSort() {
  ...
  for (i = 0; i < iW.length; i++) {
    var ind = Math.floor(Math.random() * iW.length); while (wordsG[ind].localeCompare("") != 0) {
      ind++; if (ind == iW.length) { ind = 0; }
    } wordsGL[ind] = iWL[i]; wordsG[ind] = iW[i]; imageW[ind] = imaW[i]; questionW[ind] = queW[i]; audioW[ind] = auW[i]; alterW[ind] = altW[i];
  }
}
function calcMaxWidth() {
  for (i = 0; i < wordsG.length; i++) {
    var actualSize = 0; actualSize = $.trim(aforWords(wordsG[i])).length * (48 + 9);
    if (actualSize > actMaxWidth) { actMaxWidth = actualSize }
  } $("#ardoraAct").css("width", actMaxWidth + 20 + "px");
...
function paintbGame() {
  var iW = []; for (i = 0; i < wordsGL[indexG]; i++) {
    if (aforWords(wordsG[indexG]).substr(i, 1).localeCompare("") != 0) {
      iW.push("");
    }
  } for (i = 0; i < wordsGL[indexG]; i++) {
    if (aforWords(wordsG[indexG]).substr(i, 1).localeCompare("") != 0) {
      var ind = Math.floor(Math.random() * iW.length); while (iW[ind].localeCompare("") != 0) {
        ind++;
        if (ind == iW.length) { ind = 0; }
      } iW[ind] = $.trim(aforWords(wordsG[indexG])).substr(i, 1);
    }
  }
  var newHtml = "<ul id='sortable'>"; for (i = 0; i < wordsGL[indexG]; i++) {
    if (aforWords(wordsG[indexG]).substr(i, 1).localeCompare("") != 0) {
  ...
  calcMaxWidth();
  $(".bGame").mouseenter(function () {
    $(this).css({ background: colorSele, color: colorText, borderColor: colorButton });
  ...
  $(".bGame").keydown(function (event) {
    if (event.which == 37 || event.which == 38) { $(this).insertBefore($(this).prev()); }
...
function showButtonNext() {
  ...
  contexto.globalAlpha = 0.50; contexto.fillStyle = colorBack;
  contexto.fillRect(0, 0, canvas.width, canvas.height);
  contexto.font = "14px " + fMenssage;
  var metricsW = contexto.measureText(textBNext).width;
  var x = canvas.width - metricsW - 50; var y = canvas.height - 14 - 40;
  contexto.beginPath();
  contexto.globalAlpha = 1;
  contexto.lineWidth = 2;
  contexto.fillStyle = colorButton; contexto.strokeStyle = colorSele;
  roundedRect(contexto, x, y, metricsW + 45, 14 + 20, 5, colorButton);
  contexto.shadowColor = "black"; contexto.shadowBlur = 15;
  contexto.shadowOffsetX = 4;
  contexto.shadowOffsetY = 4;
  contexto.stroke();
  contexto.beginPath();
  contexto.textAlign = "left";
  contexto.fillStyle = colorText;
  contexto.fillText(textBNext, x + 10, y + 14 + 10);
  contexto.lineWidth = 5; contexto.stroke(); contexto.fill();
  $("#ardoraActCanvas").focus();
  $("#ardoraActCanvas").bind("click keypress", function (e) {
